...
class PackagingTests(unittest.TestCase):
    def check_archive(self, archive):
        with tempfile.TemporaryDirectory() as extracted, zipfile.ZipFile(archive) as package:
            self.assertEqual(sorted(package.namelist()), ['lambda_function.py', 'payload.py'])
    ...
    def test_terraform_explicit_sources_form_an_importable_archive(self):
        ...
        self.assertEqual({name for _, name in sources}, {'lambda_function.py', 'payload.py'})
    ...
    def test_workflow_bootstrap_and_code_update_include_only_runtime_modules(self):
        ...
        self.assertIn('(cd stats_aggregator && zip ../terraform/stats_aggregator.zip lambda_function.py payload.py)', workflow)
        matrix = re.search(r'- dir: stats_aggregator\s+function_var: \w+\s+package_files: ([^\n]+)', workflow)
        self.assertIsNotNone(matrix)
        self.assertEqual(matrix.group(1).split(), ['lambda_function.py', 'payload.py'])
        for members in [['lambda_function.py', 'payload.py'], matrix.group(1).split()]:
            with tempfile.TemporaryDirectory() as directory:
