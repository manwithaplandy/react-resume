...
Controller visual inspection of b1-text-final.png found the newly visible text-view category headings and connection-kind labels faint. Include GraphListFallback informational text in B2’s graph contrast measurement and minimal style corrections needed to meet the master F17 requirement; retain all content and semantics. The shared B file map includes this component even though B2’s short file list omits it.

Deferred B1 review minor: GraphExplorer performance fallback currently unconditionally focuses the 3D mode button even when focus is on page navigation outside experienceRef. Apply the containment guard during B2 and verify an outside control keeps focus while fallback still recovers focus from inside the disappearing canvas experience.
...
F17 scan also locates FocusPanel wrapped-state text at neutral-500. Include it with actual rendered contrast. Do not mechanically lighten print-only text (graph.tsx has a neutral-600 print-only paragraph) or disabled/decorative controls: evaluate their actual background and information role. The requested target remains readable informational text and controls.

A5 shared default hook is implemented at src/hooks/useReducedMotion.ts in 3827362. Consume that exact hook; effective graph reduction remains OS/browser preference OR existing manual setting. A5 42/42 combined browser tests pass including real particle bitmap checks; actual OS-setting switching remains unverified rather than assumed.
