...
Controller read-only OS preflight: defaults read com.apple.universalaccess reduceMotion reports that the domain/default pair does not exist. No OS preference was changed, and absence of that key alone is not an observed user preference. Prior CUA inventory exposed only the in-app browser, without an authenticated/native settings surface. Distinguish Playwright media-query emulation from an actual OS-setting switch in evidence; do not claim the latter happened unless you actually perform and observe it. Preserve/restore exact pre-existing preference state if any explicitly scoped temporary OS test becomes possible.
