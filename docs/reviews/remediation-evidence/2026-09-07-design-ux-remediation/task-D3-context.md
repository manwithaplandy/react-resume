...
Primary-source verification by controller: [AWS transaction IAM guidance](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/transaction-apis-iam.html) says Put/Update/Get permissions use the underlying PutItem/UpdateItem/GetItem actions; ConditionCheck uses ConditionCheckItem. Preserve the plan's prohibition on inventing a TransactWriteItems IAM action. [AWS transaction behavior](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/transaction-apis.html) confirms all-or-nothing writes, 100 distinct items, 4 MB aggregate items, no two actions on one item, and only a 10-minute client-token idempotency window. This supports the durable chunk ledger rather than relying solely on tokens. Strongly consistent reads are still required by this plan; a multi-item Scan is not a transactional snapshot.
...
D2 review exposed how failed-source publication can mix newlystored data with prior declaredcoverage; D2fixownsCloudflareconsistentpublication. InD3, assess analogousCloudFrontpartialpassboundary: lastSuccessfulUpdate must notadvanceonincompletepass, and any permitted between-completed-input publication must truthfullyalign totals/observations/coverage. Publishing between completedinputs is optional("may") inbrief; preserving priorpublicpayload for incompletepasses is allowed ifthatavoids untruthful mixedmetadata. Neverpublishafterpartlyappliedinput. Do notinvent source-success dates ordiscardhistory toalignperiods.
