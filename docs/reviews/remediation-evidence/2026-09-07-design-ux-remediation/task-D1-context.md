...
Deterministic browser dates: the required fixtures use September 2026 dates. Fix the browser’s calendar time explicitly when checking current/stale presentation so CI does not change meaning as real days pass; preserve advancing request timers for the timeout/retry checks. Unit normalization already takes an explicit today argument. Do not alter production freshness logic to fit a test fixture.

F17 preservation check: pre-D1 stats page/StatCard/BarList informational text already uses neutral400-or-brighter on neutral900/950; no source-level color defect was established in the controller scan. Retain legibility while adding source/date/gap explanations and measure new small informational states during the planned mobile presentation check. Avoid mechanical unrelated brightening.
