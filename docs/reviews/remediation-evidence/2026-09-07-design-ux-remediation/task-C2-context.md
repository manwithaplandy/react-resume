...
Preserve the C2 distinction between submitted payload and editable draft: send a trimmed immutable snapshot, but keep the originally entered field values (including incidental whitespace) on non-success; the brief explicitly requires preserving the exact draft. Read-only pending fields ensure success can safely clear the captured attempt without overwriting newer edits. Include this observable distinction in the existing outcome tests rather than normalizing visible values on submit. C1 actual screen-reader speech was unavailable; keep manual speech verification distinct from browser semantics.
...
The inputClass currently uses placeholder:text-neutral-500 on bg-neutral-900 (source ContactForm.tsx:171 at C1 head). Confirm the actual rendered placeholder/background/opacity pair; if below 4.5:1, use the minimal supporting-text correction and preserve hierarchy. Include this in mobile screenshot/contrast evidence, no artificial prose/style unit test.
