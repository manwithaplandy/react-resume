...
await context.addInitScript({content:ts.transpileModule(init.replace(/\);$/, ')()'),{compilerOptions:{target:ts.ScriptTarget.ES2022}}).outputText});
...
writeFileSync(`${ws}/evidence/e6-graph-render.json`,JSON.stringify({results,errors},null,2));await browser.close();
